...
namespace StockSharer.Web.Data
{
    public class UserRepository
    {
        ...
        public User RetrieveUser(string email)
        {
            ...
            {
                const string sql = @"SELECT UserId, Forename, Surname, Email, Latitude, Longitude, Balance, Active FROM [User] WHERE Email = @Email";
        ...
        public User RetrieveUser(int userId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                const string sql = @"SELECT UserId, Forename, Surname, Email, Latitude, Longitude, Balance, Active FROM [User] WHERE UserId = @UserId";
        ...
        public List<User> RetrieveAllActiveUsers()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                const string sql = @"   SELECT UserId, Forename, Surname, Email, Latitude, Longitude, Balance, Active 
                                        FROM [User] 
                                        WHERE Active = 1";
        ...
        public void UpdateUser(User user)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                const string updateSql = @"UPDATE [User] SET Forename = @Forename, Surname = @Surname WHERE UserId = @UserId";
